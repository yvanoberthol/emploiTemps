...
- `classes`: A hash of classes which should be changed or enabled
...
[tooltip](https://github.com/HubSpot/tooltip/blob/master/sass/tooltip-primaryColor-arrows.sass#L14) for
an example of how to make your own prefixed css file.

All classes can be changed or enabled with the `classes` option.  For example, to change the
...
- `tether-enabled` is added to both elements when tether is not enabled
